  ...
  <div class="walletStyle">
    ...
    <div class="tableStyle">
      <el-row class="rowStyle">
        <el-button type="primary" icon="el-icon-plus" size="mini" @click="handleAdd(0)">
          新增
        </el-button>
      </el-row>
      <el-table v-loading="loading" :data="configList" border :max-height="650">
        <el-table-column label="ID" align="center" prop="id" />
        <el-table-column label="充值币种" align="center" prop="deposit_coin_type" />
        <el-table-column label="兑换币种" align="center" prop="change_coin_type" />
        <el-table-column label="允许实时汇率区间" align="center" prop="currency">
          <template slot-scope="scope">
            <span>{{ scope.row.min_range + '-'+scope.row.max_range }}</span>
          </template>
        </el-table-column>
        <el-table-column label="手动默认汇率" align="center" prop="default_num" />
        ...
        <el-table-column label="操作" align="center" fixed="right" width="200px">
          <template slot-scope="scope">
            <el-button v-hasPermi="['system:config:edit']" size="mini" @click="handleUpdateStatus(scope.row)">{{
              scope.row.status === 2 ? "上架" : "下架"
            }}</el-button>
            <el-button v-hasPermi="['system:config:edit']" size="mini" type="primary" @click="handleAdd(scope.row)">编辑
            </el-button>
            <el-button v-hasPermi="['system:config:edit']" type="danger" size="mini" @click="handleDel(scope.row)">删除
            </el-button>
          </template>
        </el-table-column>
      </el-table>
